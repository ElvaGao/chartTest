                ...
                $(".M-box2").pagination({
                    ...
                    callback: function (api) {
                        ...
                        var offerHeader = {
                            "msgtype":"S3301",
                            "sectionId": secId,
                            "startIndex": api.getCurrent() * 10 - 10,
                            "count": 10,
                            "field": 0,
                            "orderType": 0
                        };
...
function redrawChart(date,stockOne){
    ...
    if(date){
        ...
        var v_data =  getxAxis((date.Date),stockOne);
        var option ={
            yAxis: {
                type:"value",
                min: minY,
...
function initYCCharts(data,stockOne){
    ...
    var minY,middleY,maxY,minY1,maxY1;
    if (yc) {
        if(stockOne.interval!=0){
            minY = (yc - stockOne.interval).toFixed(decimal);
            middleY = yc.toFixed(decimal);
            maxY = (yc + stockOne.interval).toFixed(decimal);
        }else{
            minY = (yc - yc*0.1).toFixed(decimal);
            middleY = yc.toFixed(decimal);
            maxY = (yc + yc*0.1).toFixed(decimal);
        }
...
$(".search table").delegate(".stocklist","click",function(){
    $(this).siblings().attr("class","stocklist").end().addClass("tr-active");
    openDetail();
});
// 点击搜索
