...
(function($){
	...
	$.KLineRequire.prototype = {
		...
		// websocket应答				
		initEvent: 	function(ws, klineObj, isClicked) {
	...
	function KCharts (dataList, isHistory){
		...
		KLineData.KChart.on('showTip', function (params) {
		    ...
		    setToolInfo(length, KChartsOpt.mouseHoverPoint, 'showTip')
		});
		// 鼠标滑过，出现信息框
		...
		$("#kline_charts").bind("mouseout", function (event) {
			...
			initMarketTool(KChartsOpt);// 显示信息
		});
		// 按键操作
		$(document).keyup(function (e) {
		    ...
		    switch (keyCode) {
		        case 37:
		            move(-1, true, KLineData.KChart, KChartsOpt);
		            break; //左
		        case 38:
		            move(1,false, KLineData.KChart, KChartsOpt);
		            break;  //上
		        case 39:
		            move(1, true, KLineData.KChart, KChartsOpt);
		            break; //右
		...
		function splitData(data, isHistory) {
		    ...
		    $.each(data,function(i,object){
		        ...
		        let e_date = formatDate(object.Date),                 // 当天日期
		            ...
		            e_price = (KLineData.lineType=="day"&&(!isHistory))?floatFixedFour(object.Last):floatFixedFour(object.Price),           // 收盘价
		...
		// 初始化设置显示信息
		function initMarketTool(KChartsOpt) {
			var length = KLineData.hCategoryList.length;
			if (!KChartsOpt.isHoverGraph || KChartsOpt.isHoverGraph && !data.values[KChartsOpt.mouseHoverPoint]) {
		    	setToolInfo(length, KChartsOpt.mouseHoverPoint, null)
		    }
		};
		// 按键对应的move函数
		function move(index, type, chart, KChartsOpt) {
			var zoom = 10,
				start = 0,
				count = 0;
		    if (type) {
		...
		function setToolInfo(length, mouseHoverPoint, showTip){ 
		    var setPoint;
		    if(showTip){
		    ...
		    if (length) {
		        ...
		        $(".open", countent).text(floatFixedTwo(KLineData.hValuesList[setPoint][0])+"("+floatFixedTwo(KLineData.hValuesPercentList[setPoint][0])+"%)").attr("class",KLineData.hValuesPercentList[setPoint][0]>0?"open pull-right red":"open pull-right green"); //开
		        $(".price", countent).text(floatFixedTwo(KLineData.hValuesList[setPoint][1])+"("+floatFixedTwo(KLineData.hValuesPercentList[setPoint][1])+"%)").attr("class",KLineData.hValuesPercentList[setPoint][1]>0?"price pull-right red":"price pull-right green"); //收
		        $(".lowest", countent).text(floatFixedTwo(KLineData.hValuesList[setPoint][2])+"("+floatFixedTwo(KLineData.hValuesPercentList[setPoint][2])+"%)").attr("class",KLineData.hValuesPercentList[setPoint][2]>0?"lowest pull-right red":"lowest pull-right green"); //低
		        $(".highest", countent).text(floatFixedTwo(KLineData.hValuesList[setPoint][3])+"("+floatFixedTwo(KLineData.hValuesPercentList[setPoint][3])+"%)").attr("class",KLineData.hValuesPercentList[setPoint][3]>0?"highest pull-right red":"highest pull-right green"); //高
		        $(".z-value", countent).text(floatFixedTwo(KLineData.hZValuesList[setPoint])+"("+floatFixedTwo(KLineData.hZValuesListPercent[setPoint])+"%)").attr("class",KLineData.hZValuesList[setPoint]>0?"z-value pull-right red":"z-value pull-right green");   // 涨跌
		        $(".volume", countent).text((KLineData.hVolumesList[setPoint][1]/10000).toFixed(2)+"万"); //量
		        $(".amplitude", countent).text(floatFixedTwo(KLineData.hZf[setPoint])+"("+floatFixedTwo(KLineData.hZfList[setPoint])+"%)");   // 振幅
		        $(".price", $("#kline .kline-info")).text(floatFixedTwo(KLineData.hValuesList[setPoint][1])); //收
	...
	$(".charts-tab li").on("click",function(){
		// 取消之前的订阅
		klineObj.initEvent(ws, klineObj, "clicked");
