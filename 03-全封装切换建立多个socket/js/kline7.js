...
(function($){
						...
						ws.onmessage = function (evt) {
						    ...
						    $.each(jsons,function (i,o) {
						        if(o!==""){
						            ...
						            {
						                case "Q213":        // 订阅分钟线应答
						                 	var dataJsons = KLineData.kchartsObj.splitData(dataList);
											KLineData.kchartsObj.saveData(dataJsons);
						                ...
						                case "R213":        // 分钟K线历史数据查询
						                 	...
						                 	KLineData.kchartsObj = new $.KCharts(dataList, "history");
						                 	break;
						                case "R211":        // 日K线历史数据查询
						                	if(!KLineData.isQAlled){
						                 		socket.request(klineOpt.KQAll);	 // 订阅当前日期K线
	...
	$.KCharts = function (dataList, isHistory){
		...
		// 解析并存储数据
		var dataJsons = ele.splitData(dataList); 
		ele.saveData(dataJsons, "history");
	...
	$.KCharts.prototype = {
		splitData:	function(data) {
					    ...
					    $.each(data,function(i,object){
					        ...
					        let e_date = ele.formatDate(object.Date),                 // 当天日期
					            e_day = week[(new Date(e_date)).getDay()],        // 计算星期
					            e_time,                                           //时间
					            e_open = ele.floatFixedFour(object.Open),             // 开
					            e_highest = ele.floatFixedFour(object.High),          // 高
					            e_lowest = ele.floatFixedFour(object.Low),            // 低
					            e_price = ele.floatFixedFour(object.Price),           // 收盘价
					            ...
					            e_valuePercent = [                                // 开收低高-百分比-相对上一根柱子的收盘价
					                ele.floatFixedFour((e_open-lastClose)*100/lastClose),
					                ele.floatFixedFour((e_price-lastClose)*100/lastClose),
					                ele.floatFixedFour((e_lowest-lastClose)*100/lastClose),
					                ele.floatFixedFour((e_highest-lastClose)*100/lastClose)
					            ],

					            // e_volume = (e_price-e_open)>0?[i,object.Volume,-1]:[i,object.Volume,1],       // 成交量-数组，存储索引，值，颜色对应的值                                         // 成交量
					            e_zValues = lastClose?ele.floatFixedFour(e_price-lastClose):0,                       // 涨幅-相对昨收      
					            e_zValuesPercent = ele.floatFixedFour(e_zValues*100/lastClose),                    // 涨幅百分比
					            e_amplitude = ele.floatFixedFour(e_highest - e_lowest),                          // 振幅
					            e_amplPercent = ele.floatFixedFour(100*e_amplitude/lastClose);                     // 振幅百分比
					            if(data.length>2){
					        ...
					        switch(lineType){
					            case "minute":
					                lastClose = e_price;
					                e_time = e_date + " " + e_day + " " + ele.formatTime((object.Time/100000>=1)?object.Time:("0"+object.Time));
					                k_categoryData.push(e_time);
					                break;
					            case "day":
					                lastClose = e_price;
					                hTime = ele.formatTime((object.Time/100000>=1)?object.Time:("0"+object.Time));
						        ...
						        switch(lineType){
						            case "minute":
						                $(".time", countent).text(KLineData.hCategoryList[setPoint].split(" ")[2]); //时间
						                 break;
						            case "day":
						                $(".time", countent).text((mouseHoverPoint==length-1)?KLineData.hTime:null); //时间
						        }
						        $(".open", countent).text(ele.floatFixedTwo(KLineData.hValuesList[setPoint][0])+"("+ele.floatFixedTwo(KLineData.hValuesPercentList[setPoint][0])+"%)").attr("class",KLineData.hValuesPercentList[setPoint][0]>0?"open pull-right red":"open pull-right green"); //开
						        $(".price", countent).text(ele.floatFixedTwo(KLineData.hValuesList[setPoint][1])+"("+ele.floatFixedTwo(KLineData.hValuesPercentList[setPoint][1])+"%)").attr("class",KLineData.hValuesPercentList[setPoint][1]>0?"price pull-right red":"price pull-right green"); //收
						        $(".lowest", countent).text(ele.floatFixedTwo(KLineData.hValuesList[setPoint][2])+"("+ele.floatFixedTwo(KLineData.hValuesPercentList[setPoint][2])+"%)").attr("class",KLineData.hValuesPercentList[setPoint][2]>0?"lowest pull-right red":"lowest pull-right green"); //低
						        $(".highest", countent).text(ele.floatFixedTwo(KLineData.hValuesList[setPoint][3])+"("+ele.floatFixedTwo(KLineData.hValuesPercentList[setPoint][3])+"%)").attr("class",KLineData.hValuesPercentList[setPoint][3]>0?"highest pull-right red":"highest pull-right green"); //高
						        $(".z-value", countent).text(ele.floatFixedTwo(KLineData.hZValuesList[setPoint])+"("+ele.floatFixedTwo(KLineData.hZValuesListPercent[setPoint])+"%)").attr("class",KLineData.hZValuesList[setPoint]>0?"z-value pull-right red":"z-value pull-right green");   // 涨跌
						        $(".volume", countent).text((KLineData.hVolumesList[setPoint][1]/10000).toFixed(2)+"万"); //量
						        $(".amplitude", countent).text(ele.floatFixedTwo(KLineData.hZf[setPoint])+"("+ele.floatFixedTwo(KLineData.hZfList[setPoint])+"%)");   // 振幅
						        $(".price", $("#kline .kline-info")).text(ele.floatFixedTwo(KLineData.hValuesList[setPoint][1])); //收
						        $(".z-value", $("#kline .kline-info")).text(ele.floatFixedTwo(KLineData.hZValuesListPercent[setPoint])+"%"); //收
						    }else{
						        $(".name", countent).text("-");
						        $(".date", countent).text("-");
						        $(".day", countent).text("-");
	...
	$.queryKLine = function(opt) {
    	...
    	var klineObj = new $.KLineRequire({"InstrumentID": id});
		// 请求xml数据，获取指数名称
