...
(function($){
	...
	// 初始化请求参数
	$.KLineRequire = function(option, klineType){
	...
	$.KLineRequire.prototype = {
		// websocket请求连接
		WebSocketConnect: 	function (requireObj) {	
								this.ws = null;
								...
								this.createWebSocket = function () {
								    try {
								        this.ws = new WebSocket(requireObj.wsUrl);
								        return this.ws;
								    } catch (e) {
								        this.reconnect(requireObj.wsUrl); //如果失败重连
								    }
								};
								//socket重连
								this.reconnect = function (wsUrl) {
								    ...
								    setTimeout(function () {
								        ...
								        requireObj.initEvent(ws, requireObj);
								...
								this.start = function () {
								    ...
								    this.timeoutObj = setTimeout(function () {
								        ...
								        self.request(requireObj.HeartSend);
		...
		// websocket应答				
		initEvent: 	function(ws, requireObj) {
						var klineOpt = this?this:"";
						ws.onclose = function () {
							console.log("终端重连……");
						    socket.reconnect(requireObj.wsUrl); //终端重连
						};
						ws.onerror = function () {
							console.log("报错重连……");
						    socket.reconnect(requireObj.wsUrl); //报错重连
						};
						ws.onopen = function () {
							console.log("open");
						    //心跳检测重置
	...
	$.queryKLine = function(option) {
		...
		$(".charts-tab li").on("click",function(){
			...
			if(KLineData.lineType=="mline"&&turnOn){
				...
				if(turnOn){
					// Websocket实例
				    socket = new requireObj.WebSocketConnect(requireObj);
